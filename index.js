...
app.get('/orders', async function (req, res, next){
  try {
    ...
    const { data, status } = await axios({
      url: `${config.apiUrl}/api/orders?searchBy=${search}&pageNumber=${pageNumber}&nPerPage=${nPerPage}`,
      headers: { Authorization: `Bearer ${token}` },
      method: 'get',
      responseType: 'json',
    });
...
app.get('/users', async function (req, res, next) {
  try {
    ...
    const nPerPage = req.query.nPerPage;
    const { data, status } = await axios({
      url: `${config.apiUrl}/api/users?searchBy=${search}&pageNumber=${pageNumber}&nPerPage=${nPerPage}`,
      headers: { Authorization: `Bearer ${token}` },
      method: 'get',
      responseType: 'json',
    });
